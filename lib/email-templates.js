...
import { formatTimeInTimezones } from "./timezone-helper";
...
export function adminOtpEmail({ otp, settings, siteUrl }) {
  const content = `
    ...
    <p style="margin:0;font-size:13px;line-height:1.6;color:${BRAND.muted};text-align:center;">
      This code is valid for <b style="color:${BRAND.ink};">10 minutes</b>. If you did not request this, please change your admin password immediately.
    </p>`;
  return {
    subject: "Your Admin Verification Code — Yaqeen Institute",
    html: shell({ preheader: `Your admin verification code is ${otp} (valid 10 minutes).`, contentHtml: content, settings, siteUrl }),
    text: `Your Yaqeen Institute admin verification code is: ${otp}\nThis code is valid for 10 minutes. If you did not request this, change your password immediately.`,
  };
}
...
<a href="https://wa.me/447488848483?text=Assalamu+Alaikum%21+I%E2%80%99m+interested+in+learning+more+about+Yaqeen+Institute+and+your+courses.">+44 7488 848483</a>
</div>
...
export function freeTrialAdminEmail({ form, settings, siteUrl, meta }) {
  ...
  let timeHtml = escapeHtml(time || "TBD");
  const tzRes = form.convertedTimes || (form.date && form.hh && form.mm && form.ap ? formatTimeInTimezones(form.date, form.hh, form.mm, form.ap, form.timezone, form.country) : null);
  if (tzRes && tzRes.success) {
    timeHtml = `
      <ul style="margin: 0; padding: 0 0 0 16px; list-style-type: disc;">
        <li style="margin-bottom: 4px;">${escapeHtml(tzRes.local)}</li>
        <li style="margin-bottom: 4px;">${escapeHtml(tzRes.india)}</li>
        <li style="margin-bottom: 0;">${escapeHtml(tzRes.egypt)}</li>
      </ul>
    `;
  }
...
<tr><td>Preferred Time</td><td>${timeHtml}</td></tr>
...
export function newsletterAdminEmail({ email, ip = "", location = "", source = "Website Footer" }) {
  const html = `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<style>
body{margin:0;padding:0;background-color:#F5EBDD;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;color:#2B1F14;}
.wrapper{width:100%;background-color:#F5EBDD;padding:32px 16px;}
.container{max-width:560px;margin:0 auto;background:#FFFFFF;border-radius:16px;overflow:hidden;border:1px solid #EADDC8;box-shadow:0 8px 30px rgba(43,31,20,0.06);}
.header{background:linear-gradient(135deg,#556B3B 0%,#3C4E28 100%);padding:28px 24px;text-align:center;}
.header img{height:46px;width:auto;display:block;margin:0 auto;}
.content{padding:30px 28px;}
h1{margin:0 0 16px 0;font-size:22px;color:#2B1F14;font-weight:700;}
p{margin:0 0 14px 0;font-size:14px;line-height:1.6;color:#5C4D3C;}
.card{background:#FAF8F5;border:1px solid #F5EBDD;border-radius:12px;padding:18px 20px;margin:20px 0;}
.row{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #F0E4D2;font-size:13.5px;}
.row:last-child{border-bottom:none;}
.label{color:#7C7267;font-weight:500;}
.val{color:#2B1F14;font-weight:700;word-break:break-all;}
.badge{display:inline-block;background:rgba(201,155,77,0.15);color:#8C5D31;border:1px solid rgba(201,155,77,0.3);padding:3px 10px;border-radius:99px;font-size:12px;font-weight:700;}
.footer{background:#FAF8F5;padding:20px;text-align:center;font-size:11.5px;color:#7C7267;border-top:1px solid #F5EBDD;}
</style>
</head>
<body>
<div class="wrapper">
  <div class="container">
    <div class="header">
      <img src="https://res.cloudinary.com/az6stpg2/image/upload/v1783424854/yaqeenlogo.png" alt="Yaqeen Institute">
    </div>
    <div class="content">
      <h1>📬 New Newsletter Subscriber</h1>
      <p>A new visitor has subscribed to the Yaqeen Institute newsletter:</p>
      
      <div class="card">
        <div class="row"><span class="label">Subscriber Email:</span><span class="val" style="color:#C99B4D;font-size:15px;">${escapeHtml(email)}</span></div>
        <div class="row"><span class="label">Subscription Source:</span><span class="val"><span class="badge">${escapeHtml(source)}</span></span></div>
        ${location ? `<div class="row"><span class="label">Location:</span><span class="val">${escapeHtml(location)}</span></div>` : ""}
        ${ip ? `<div class="row"><span class="label">IP Address:</span><span class="val">${escapeHtml(ip)}</span></div>` : ""}
        <div class="row"><span class="label">Date & Time:</span><span class="val">${new Date().toUTCString()}</span></div>
      </div>
      
      <p style="margin-top:16px;font-size:13px;color:#7C7267;">
        You can view, search, export, and manage all newsletter subscribers from your Admin Dashboard.
      </p>
    </div>
    <div class="footer">
      © 2026 Yaqeen Institute Admin Notification System
    </div>
  </div>
</div>
</body>
...
export function newsletterUserEmail({ email, settings = {}, siteUrl = "https://yaqeeninstitute.online" }) {
  const html = `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
...
body{margin:0;padding:0;background-color:#F5EBDD;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;color:#2B1F14;}
.wrapper{width:100%;background-color:#F5EBDD;padding:32px 16px;}
.container{max-width:580px;margin:0 auto;background:#FFFFFF;border-radius:16px;overflow:hidden;border:1px solid #EADDC8;box-shadow:0 8px 30px rgba(43,31,20,0.06);}
.header{background:linear-gradient(135deg,#556B3B 0%,#3C4E28 100%);padding:30px 24px;text-align:center;}
.header img{height:50px;width:auto;display:block;margin:0 auto;}
.content{padding:32px 28px;}
h1{margin:0 0 16px 0;font-size:24px;color:#2B1F14;font-weight:700;line-height:1.25;}
p{margin:0 0 14px 0;font-size:14.5px;line-height:1.65;color:#5C4D3C;}
.highlight-box{background:#FAF8F5;border-left:4px solid #C99B4D;border-radius:0 12px 12px 0;padding:16px 18px;margin:20px 0;}
.highlight-box p{margin:0;color:#2B1F14;font-weight:500;font-size:14px;}
.benefits-list{list-style:none;padding:0;margin:18px 0;}
.benefits-list li{display:flex;align-items:flex-start;gap:10px;padding:6px 0;font-size:14px;color:#4A3B2C;}
.btn-gold{display:inline-block;background:#C99B4D;color:#FFFFFF;text-decoration:none;padding:13px 32px;border-radius:9999px;font-weight:600;font-size:14.5px;box-shadow:0 4px 14px rgba(201,155,77,0.3);margin-top:14px;}
.footer{background:#FAF8F5;padding:24px;text-align:center;font-size:12px;color:#7C7267;border-top:1px solid #F5EBDD;}
.footer a{color:#556B3B;text-decoration:underline;margin:0 6px;}
</style>
</head>
<body>
<div class="wrapper">
  <div class="container">
    ...
    <div class="content">
      <h1>Assalamu Alaikum & Welcome! 🌟</h1>
      <p>JazakAllah Khair for subscribing to the <strong>Yaqeen Institute</strong> newsletter.</p>
      
      <div class="highlight-box">
        <p>You are now part of our global learning family! Here is what you can look forward to receiving:</p>
      </div>

      <ul class="benefits-list">
        <li>✨ <strong>Free Quran & Tajweed Guides</strong> to help improve your daily recitation</li>
        <li>📖 <strong>Beneficial Islamic Articles & Reminders</strong> from qualified tutors</li>
        <li>🎯 <strong>Exclusive Offers & Early Access</strong> to new courses and webinars</li>
        <li>💡 <strong>Tips for Kids and Family Learning</strong> in a supportive online environment</li>
      </ul>

      <p>Whether you're starting your Quran journey from the basics or advancing with Tajweed and Memorization, we're here to support you at every step.</p>

      <div style="text-align:center;margin:24px 0 10px 0;">
        <a href="${escapeHtml(siteUrl)}/register" class="btn-gold">Book a Free Trial Class →</a>
      </div>
    </div>
    <div class="footer">
      <p style="margin-bottom:8px;">Need assistance? Reach us anytime at <a href="mailto:support@yaqeeninstitute.online">support@yaqeeninstitute.online</a></p>
      <div style="margin-top:10px;">
        <a href="${escapeHtml(siteUrl)}">Home</a> |
        <a href="${escapeHtml(siteUrl)}/courses">Courses</a> |
        <a href="${escapeHtml(siteUrl)}/privacy">Privacy Policy</a>
      </div>
      <p style="margin-top:14px;font-size:11px;color:#A69A86;">© 2026 Yaqeen Institute. All rights reserved.</p>
    </div>
  </div>
</div>
</body>
  ...
  return {
    subject: `Welcome to Yaqeen Institute Newsletter! ✨`,
    html,
    text: `Assalamu Alaikum!\n\nJazakAllah Khair for subscribing to the Yaqeen Institute newsletter.\n\nYou will now receive free Quran resources, Tajweed guides, Islamic articles, and course updates.\n\nVisit us: ${siteUrl}\nBook a Free Trial: ${siteUrl}/register\n\n© 2026 Yaqeen Institute`,
  };
}
