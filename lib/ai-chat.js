...
// Whole-word (or whole-phrase) match so short keywords don't match inside
// unrelated words — e.g. "try" must not match inside "coun-try".
const escapeRe = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
const matchWord = (text, w) => {
  if (!w) return false;
  return new RegExp(`(^|[^a-z0-9])${escapeRe(String(w).toLowerCase())}([^a-z0-9]|$)`, "i").test(text);
};
const has = (text, words) => words.some((w) => matchWord(text, w));
...
async function matchAdminQa(text, supabase) {
  try {
    ...
    for (const row of data) {
      ...
      let score = 0;
      for (const t of new Set(tokens)) if (matchWord(text, t)) score++;
