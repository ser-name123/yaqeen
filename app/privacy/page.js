...
export async function generateMetadata() {
  return pageMetadata(getSupabaseAdmin(), "privacy");
}
...
export default async function PrivacyPolicyPage() {
  const c = await getPageContent(getSupabaseAdmin(), "privacy");
  ...
  const tocItems = [
    ...
    { id: `s${contactNum}`, label: `${contactNum}. ${c.contact?.heading || "Contact Information"}` }
  ];
      ...
      <div className="pp-layout">
        ...
        <article className="pp-content">
          ...
          <section id={`s${contactNum}`} className="pp-section">
            <h2><span className="num">{contactNum}.</span> {c.contact?.heading || "Contact Information"}</h2>
            <p>{c.contact?.intro}</p>
