...
export async function generateMetadata() {
  return pageMetadata(getSupabaseAdmin(), "cookies");
}
...
export default async function CookiesPolicyPage() {
  const c = await getPageContent(getSupabaseAdmin(), "cookies");
  ...
  const tocItems = [
    ...
    { id: `s${contactNum}`, label: `${contactNum}. ${c.contact?.heading || "Contact Us"}` }
  ];
      ...
      <div className="pp-layout">
        ...
        <article className="pp-content">
          ...
          <section id={`s${contactNum}`} className="pp-section">
            <h2><span className="num">{contactNum}.</span> {c.contact?.heading || "Contact Us"}</h2>
            <p>{c.contact?.intro}</p>
