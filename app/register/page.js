...
import "../book-free-trial/book-free-trial.css";
    ...
    async function detectCountry() {
      ...
      if (res && res.ok) {
        try {
          ...
          console.warn("Failed to parse detect-country API response on register page", e);
        }
      }
    }
    detectCountry();
    ...
    return (
      <main className="bft-page">
        <section className="bft-hero">
          <span className="bft-badge"><IconSpark /> {c.hero?.badge || "100% Free · No Credit Card Required"}</span>
          <h1 className="bft-hero-title">
            {c.hero?.title_prefix || "Book Your "}<span>{c.hero?.title_highlight || "Free Trial"}</span>{c.hero?.title_suffix || " Class"}
          </h1>
          ...
          <p className="bft-hero-sub" style={{ textAlign: "center", marginTop: "10px" }}>
            {c.hero?.loading_text || "Loading booking form…"}
          </p>
        </section>
      </main>
    );
    ...
    <main className="bft-page">
      ...
      <section className="bft-hero">
        ...
        <div className="bft-hero-stats">
          {(c.hero?.stats || ["1-on-1 Live Class", "30-Min Free Session", "Expert Male & Female Tutors"]).map((stat, i) => {
            const icons = [<IconCheck key="0" />, <IconClock key="1" />, <IconTeacher key="2" size={16} />];
        ...
        <div className="bft-form-card" ref={formCardRef}>
          {submitted ? (
            <div className="bft-success">
              ...
              <h3>{c.success?.title || "Booking Confirmed!"}</h3>
              <p>
                Thank you, {form.firstName}{c.success?.message_suffix || ". Our team will reach out shortly."}
              </p>
              <div style={{ display: "flex", gap: "12px", justifyContent: "center", flexWrap: "wrap" }}>
                <Link href="/" className="bft-btn bft-btn-secondary" style={{ textDecoration: "none" }}>{c.success?.home_label || "Return to Home"}</Link>
                <button type="button" className="bft-btn bft-btn-primary" onClick={resetForm}>{c.success?.another_label || "Book Another Session"}</button>
              </div>
            </div>
          ) : (
            <form onSubmit={handleSubmit} noValidate>
              ...
              <div className="bft-progress">
                <div className="bft-progress-labels">
                  <span className={`bft-progress-label ${step >= 1 ? "active" : ""}`}>{c.progress?.step1_label || "Step 1: Your Details"}</span>
                  <span className={`bft-progress-label ${step >= 2 ? "active" : ""}`}>{c.progress?.step2_label || "Step 2: Class Preferences"}</span>
                </div>
              ...
              {step === 1 && (
                <div>
                  <h2 className="bft-form-title">{c.step1?.title || "Tell Us About Yourself"}</h2>

                  <div className="bft-row">
                    <div className="bft-field" style={{ marginBottom: 0 }}>
                      <label className="bft-label">{c.step1?.first_name_label || "First Name"}<span className="req">*</span></label>
                      <input
                    ...
                    <div className="bft-field" style={{ marginBottom: 0 }}>
                      <label className="bft-label">{c.step1?.last_name_label || "Last Name"}<span className="req">*</span></label>
                      <input
                  ...
                  <div className="bft-field" style={{ marginTop: 20 }}>
                    <label className="bft-label">{c.step1?.email_label || "Email Address"}<span className="req">*</span></label>
                    <input
                      type="email"
                  ...
                  <div className="bft-field">
                    <label className="bft-label">{c.step1?.phone_label || "Phone Number"}<span className="req">*</span></label>
                  ...
                  <div className="bft-field">
                    <label className="bft-label">{c.step1?.country_label || "Country of Residence"}<span className="req">*</span></label>
                    <CustomSelect
                      value={form.country}
                  ...
                  <div className="bft-actions end">
                    <button type="button" className="bft-btn bft-btn-primary" onClick={goNext}>
                      {c.step1?.next_label || "Next Step"} <IconArrow />
                    </button>
                  </div>
                </div>
              )}
              ...
              {step === 2 && (
                <div>
                  <h2 className="bft-form-title">{c.step2?.title || "Class Preferences"}</h2>

                  <div className="bft-field">
                    <label className="bft-label">{c.step2?.learn_label || "What would you like to learn?"}<span className="req">*</span></label>
                  ...
                  <div className="bft-field">
                    <label className="bft-label">{c.step2?.session_label || "Who is this trial class for?"}<span className="req">*</span></label>
                  ...
                  <div className="bft-field">
                    <label className="bft-label">{c.step2?.teacher_label || "Preferred Teacher Gender"}<span className="req">*</span></label>
                  ...
                  <div className="bft-field">
                    <label className="bft-label">{c.step2?.source_label || "How did you hear about us?"}</label>
                  ...
                  <div className="bft-row" style={{ alignItems: "flex-start" }}>
                    <div className="bft-field" style={{ marginBottom: 0 }}>
                      <label className="bft-label">{c.step2?.date_label || "Preferred Date"}<span className="req">*</span></label>
                      <input
                        type="date"
                    ...
                    <div className="bft-field" style={{ marginBottom: 0 }}>
                      <label className="bft-label">{c.step2?.time_label || "Preferred Time"}<span className="req">*</span></label>
                  ...
                  <div className="bft-actions">
                    <button type="button" className="bft-btn bft-btn-secondary" onClick={goPrev}>{c.step2?.prev_label || "Back"}</button>
                    <button type="submit" className="bft-btn bft-btn-primary" disabled={submitting}>
                      {submitting ? "Submitting…" : (c.step2?.submit_label || "Confirm Free Trial Booking")} {!submitting && <IconArrow />}
                    </button>
                  </div>
                </div>
              )}
